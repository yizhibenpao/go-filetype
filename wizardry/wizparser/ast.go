...
func (k Kind) String() string {
	...
	case KindFamilyGUID:
		sk, _ := k.Data.(*GUIDKind)
		return fmt.Sprintf("string    %s", strconv.Quote(string(sk.Value)))
	case KindFamilyPString:
		sk, _ := k.Data.(*PStringKind)
		return fmt.Sprintf("pstring    %s", strconv.Quote(string(sk.Value)))
	case KindFamilyRegex:
		sk, _ := k.Data.(*RegexKind)
		return fmt.Sprintf("regex/0x%x    %s", sk.MaxLen, strconv.Quote(string(sk.Value)))
...
const (
	...
	KindFamilyGUID
	// KindFamilyGUID succeeds if no tests succeeded before on that level
)
