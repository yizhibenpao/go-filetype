...
func (ctx *InterpretContext) identifyInternal(sr *wizutil.SliceReader, pageOffset int64, page string, swapEndian bool) ([]string, error) {
	...
	for _, rule := range ctx.Book[page] {
		...
		case wizparser.KindFamilyGUID:
			...
			matchLen := wizardry.GUIDTest(sr, lookupOffset, string(sk.Value))
			success = matchLen >= 0
			if success {
				globalOffset = lookupOffset + int64(matchLen)
			}
		...
		case wizparser.KindFamilyRegex:
			...
			matchPos := wizardry.RegexTest(sr, lookupOffset, sk.MaxLen, string(sk.Value), sk.Flags)
			success = matchPos >= 0

			if success {
				globalOffset = lookupOffset + matchPos //regex最后的结尾
			}
		...
		case wizparser.KindFamilyPString:
			...
			matchPos := wizardry.PStringTest(sr, lookupOffset, sk.LengthByte, string(sk.Value), sk.EndiannessBIG, sk.ContainsOwnLength)
			success = matchPos >= 0
